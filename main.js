...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("895614760723296316").send(embed); // айди вашего канала с логами
})
...
client.on('guildMemberAdd', member =>{ // ивент, когда пользователь присоединяется к серверу https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
    ...
    Ты наш \`${client.guilds.get("895549069915721728").memberCount}\` участник! **`) // айди вашего сервера               !!!!!!!!!!
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('895684836650532904').send(embed2) // айди вашего канала с логами
})
...
client.on('guildMemberRemove', member => { // ивент, когда пользователь выходит с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberRemove
    ...
    .setColor('RANDOM')
    member.send(embed);
    client.channels.cache.get('895684836650532904').send(embed) // айди вашего канала с логами
  })

async function change() {
    let members = client.guilds.cache.get("895549069915721728").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("895549069915721732").setName(`На сервере: ${members}`); // свой айди войса
}

var interval = setInterval(function () { change(); }, 20000  ); // время обновления в миллисекундах

client.login('OTE3MzIxOTA1OTY5Njk2ODE4.GtPLg2.6cuqHYEshuuRbbHC0flKbAQYN1VLEJNqIllXSU') // токен вашего бота
